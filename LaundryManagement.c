...
void simpanDataPelanggan(Pelanggan pelanggan[], int jumlah_pelanggan) {
    FILE *file;
    file = fopen("Data_Pelanggan_Laundry.txt", "w"); // Open file to write data
...
void simpanDataPesanan(Pesanan pesanan[], int jumlah_pesanan) {
    FILE *file;
    file = fopen("Data_Pesanan_Laundry.txt", "w"); // Open file to write data
...
int bacaDataPelanggan(Pelanggan pelanggan[]) {
    ...
    file = fopen("Data_Pelanggan_Laundry.txt", "r"); // Open file to read data
...
int bacaDataPesanan(Pesanan pesanan[]) {
    ...
    file = fopen("Data_Pesanan_Laundry.txt", "r"); // Open file to read data
...
// Fungsi pencarian pelanggan berdasarkan nama Metode Pencarian (Searching) - Contoh Implementasi Linear Search
...
// Fungsi untuk menukar data pesanan pada proses pengurutan Metode Pengurutan (Sorting) - Contoh Implementasi Bubble Sort untuk Pengurutan Pesanan Laundry berdasarkan Total Biaya
...
void DelayPrintPilihan(const char *text, int delayTime) {
    while (*text != '\0') {
        printf(WARNA_PATMA"%c"WARNA_RESET, *text);
        Sleep(delayTime);
        text++;
    }
}
    ...
    do {
        ...
        switch (pilihan) {
            ...
            case 7:
                ...
                printf(WARNA_SIAN"Pesanan berhasil diurutkan berdasarkan total biaya.\n");
                getch();
                system("cls");
                break;
            case 8:
